...
The examples below load the package with `box::use(assert)` and call functions through the namespace, as in `assert$assert_numeric(x)`, so it is always clear where each function comes from. `library(assert)` followed by bare `assert_numeric(x)` works too.
