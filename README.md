...
`assert_scalar_character()` accepts any single string, including `""`.
For arguments such as identifiers or codes where an empty value is never
valid, pass `non_empty = TRUE`:
